...
def change_tts_inference(if_tts,bert_path,cnhubert_base_path,gpu_number,gpt_path,sovits_path):
    ...
    if(if_tts==True and p_tts_inference==None):
        ...
        yield i18n("TTS推理进程已开启，请耐心等待界面出现...")
        print(cmd)
...
def open_asr(asr_inp_dir):
    ...
    if(p_asr==None):
        ...
        print(cmd)
        yield "语音识别任务开启：%s"%cmd,{"__type__":"update","visible":False},{"__type__":"update","visible":True}
        print(cmd)
        p_asr = Popen(cmd, shell=True)
        p_asr.wait()
        p_asr=None
        yield "语音识别任务完成",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
    else:
        yield "已有正在进行的语音识别任务，需先终止才能开启下一次任务",{"__type__":"update","visible":False},{"__type__":"update","visible":True}
...
def close_asr():
    ...
    if(p_asr!=None):
        ...
        p_asr=None
    return "已终止语音识别进程",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
...
def open1abc(inp_text,inp_wav_dir,exp_name,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,ssl_pretrained_dir,pretrained_s2G_path):
    ...
    if (ps1abc == []):
        ...
        try:
            ...
            ps1abc = []
            yield "全部进程结束", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}
        except:
            traceback.print_exc()
            close1abc()
            yield "全部中途报错", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}
    else:
        yield "已有正在进行的全部任务，需先终止才能开启下一次任务", {"__type__": "update", "visible": False}, {"__type__": "update", "visible": True}
    ...
    if (ps1abc != []):
        for p1abc in ps1abc:
            try:
                ...
                traceback.print_exc()
        ps1abc=[]
    return "已终止全部进程", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}

with gr.Blocks(title="语音处理与模型训练",theme=gr.themes.Soft()) as app:
    with gr.Tabs():
        with gr.TabItem(i18n("前置数据集获取工具")):#提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            gr.Markdown(value=i18n("UVR5人声伴奏分离&去混响去延迟工具"))
            with gr.Row():
            ...
            with gr.Row():
                with gr.Row():
                    slice_inp_path=gr.Textbox(label=i18n("音频自动切分输入路径，文件/文件夹"),value="")
                    ...
                    min_length=gr.Textbox(label=i18n("min_length:每段最小长度"),value="4000")
                    min_interval=gr.Textbox(label=i18n("min_interval:最短切割间隔"),value="300")
                    hop_size=gr.Textbox(label=i18n("hop_size:音量曲线，越小精度越大计算量越高（不是精度越大效果越好）"),value="10")
            ...
            gr.Markdown(value=i18n("中文批量离线语音识别工具"))
            with gr.Row():
                open_asr_button = gr.Button(i18n("开启离线批量语音识别"), variant="primary",visible=True)
                close_asr_button = gr.Button(i18n("终止语音识别进程"), variant="primary",visible=False)
                asr_inp_dir = gr.Textbox(
                    label=i18n("批量语音识别(中文only)输入文件夹路径"),
                    value="",
                    interactive=True,
                )
        ...
        with gr.TabItem(i18n("GPT-SoVITS-TTS")):
            with gr.Row():
                exp_name = gr.Textbox(label=i18n("要制作模型的英文名字"), value="", interactive=True)
                gpu_info = gr.Textbox(label=i18n("GPU信息"), value=gpu_info, visible=True, interactive=False)
            ...
            with gr.TabItem(i18n("训练集格式化工具")):
                gr.Markdown(value=i18n("格式化你之前处理的数据集。注意：模型名字必须是英文！！！"))
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("文本标注文件"),value=r"",interactive=True)
                ...
                gr.Markdown(value=i18n("SSL自监督特征提取"))
                with gr.Row():
                ...
                gr.Markdown(value=i18n("语义token提取"))
                with gr.Row():
                ...
                gr.Markdown(value=i18n("全部流程控制"))
                with gr.Row():
                    button1abc_open = gr.Button(i18n("开启全部"), variant="primary",visible=True)
                    button1abc_close = gr.Button(i18n("终止全部"), variant="primary",visible=False)
                    info1abc=gr.Textbox(label=i18n("全部进程输出信息"))
            ...
            with gr.TabItem(i18n("微调训练")):
                gr.Markdown(value=i18n("输出目录: /SoVITS_weights/"))
                with gr.Row():
                    batch_size = gr.Slider(minimum=1,maximum=40,step=1,label=i18n("每张GPU的batch_size"),value=default_batch_size,interactive=True)
                ...
                gr.Markdown(value=i18n("GPT训练。用于分享的模型文件输出在GPT_weights下。"))
                with gr.Row():
                    batch_size1Bb = gr.Slider(minimum=1,maximum=40,step=1,label=i18n("每张GPU的batch_size"),value=default_batch_size,interactive=True)
            ...
            with gr.TabItem(i18n("推理")):
                gr.Markdown(value=i18n("测试SoVITS_weights和GPT_weights下的模型。直接勾选以试用示例模型。"))
                with gr.Row():
                ...
                with gr.Row():
                    if_tts = gr.Checkbox(label=i18n("开启TTS推理WebUI"), show_label=True)
